...
double myrand2(){
    double r = (double)rand()/(RAND_MAX)*0.2;
    return r;                               //Doublerand
}
...
int main(void) {
    FILE *fptr, *fptr2;
    fptr = fopen("/home/GLI/jimmy.neitzert/Praktikum6Kunden.txt","w+");
    fptr2 = fopen("/home/GLI/jimmy.neitzert/Praktikum6Mittelwert.txt","w+");
    int anzahl = 0;
    ...
    event_store(0, 0.1);
    event_store(100, 1.99);
  //  for (n = 0; n < 3; n++) {
  //      event_store(rand() % 3, myrand2() * 10); //rand0-2,myrand*10 -> 10er Schritte
  //  }


    do {
        event_store(rand() % 3, myrand2() * 10);
        event_retrieve(&type, &zeit);
        //for(n=0;n<100;n++){
        anzahl++;
        switch (type) {
            ...
            case ANKUNFT:
                ...
                if (kundenmenge <= 0)
                { Q = queue_create("Kasse", 15);
               // printf("Kasse Öffnet, Bedienung wird eingetragen!\n");
                    }//Bedienung
                P= packet_create("Kunde");
              queue_store(Q, P);
                kundenmenge++;
              //  printf("(ANKUNFT) Kunde Eingereiht\n");
               if (Q->entries == 1){
                   zeit = myrand()* 1.8;
                   event_store(VERLASSEN,zeit);}
                break;
            ...
            case VERLASSEN:
             ...
             //   if (kundenmenge== 0)
             //   queue_destroy(Q);
              //  printf("(VERLASSEN) Kunde bedient\n");

                break;
            case MESSUNG:
                fprintf(fptr,"%d\n",Q->entries);
                fprintf(fptr2,"%g\n",gl_mean())
            // event_store(MESSUNG,myrand2()/1000);
               // printf("(MESSUNG)Queue hat %i einträge,%li kunden sind verloren gegangen(größe%i)\n", kundenmenge, Q->lost,
               //        Q->size);
                break;
            case 100:
                fclose(fptr);
  ...
  //  {printf("q->%d",Q->entries);
  //  printf("q->%li",Q->lost);
  //  printf("q->%d",Q->size);
  //  printf("q->%s",Q->name);
  //  printf("q->%d",Q->read);
  //  printf("q-> %d",Q->write);
  //  printf("q->time %li ",Q->time);
  fclose(fptr);
}
